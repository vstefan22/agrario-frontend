...
import { landownerSupportOptions, developerSupportOptions } from "../constants/select-options";
  ...
  return (
    <div className="bg-gray-100 min-h-screen flex flex-col px-7 py-4">
      ...
      <div className="flex flex-col bg-white rounded-[16px] shadow-[6px_6px_54px_0px_#0000000D] px-8 py-6 border-[1px] border-gray-medium/60 ">
        <h1 className="text-[32px] text-black-muted mb-4">Neue Nachricht an Agrario Support</h1>
        <Select
          name="subject"
          options={userRole === "landowner" ? landownerSupportOptions : developerSupportOptions}
        ...
        <div className="mt-4 flex justify-between items-end">
          ...
          <div className="flex gap-x-6">
            ...
            <Button variant="bluePrimary" type="button" onClick={handleSendMessage}>
              Nachricht absenden
            </Button>
          </div>
        </div>
      ...
      {messages.length > 0 ? (
        messages.map((message) => <MessageHistoryCard user="Chris" message={message.body} />)
      ) : (
