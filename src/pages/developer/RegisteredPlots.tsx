...
import { filterPlotsSearchData, sortPlotsSearchData } from "../../utils/helper-functions";
...
const RegisteredPlots = () => {
  ...
  console.log(registeredPlots);
  const searchFilteredData = filterPlotsSearchData(registeredPlots, searchTerm);
  const rangeFilteredData = filterPlotSearchDataRange(searchFilteredData, range);
  const sortedData = sortPlotsSearchData(rangeFilteredData, filters.sortOption);

  return (
    <div className="bg-gray-100 min-h-screen flex flex-col px-7 pt-4">
      <h1 className="text-[32px] font-bold text-black-muted">Flurstücke suchen</h1>
          ...
          <div className="relative">
            ...
            {isOpen && (
              <div className="absolute left-1/2 top-full transform -translate-x-1/2 px-4 py-6 rounded-2xl bg-white shadow-xl z-50">
                <h1 className="text-[32px] text-black-muted px-4">Filtereinstellungen</h1>
                <RangeSlider
                  title="Fläche"
                  ...
                  max={250}
                />
                <p className="text-[18px] text-black-muted my-4">Nutzung laut ALKIS</p>
                ...
                <div className="grid grid-cols-2 gap-y-3 gap-x-[4.6rem]">
                  ...
                  <Checkbox label="Vegetation: Landwirtschaft" variant="primary" />
                ...
                <div className="grid grid-cols-2 gap-y-3">
                  ...
                  <Checkbox label="Ökologische Aufwertungen" variant="primary" />
                </div>
                ...
                <div className="flex gap-4 mt-8">
                  <Button type="button" variant="blueSecondary" className="max-w-[204px]">
                    Filter zurücksetzen
                  </Button>
                  <Button type="button" variant="bluePrimary" className="max-w-[204px]">
                    Filter anwenden
                  </Button>
                </div>
              </div>
      ...
      {sortedData.length > 0 ? (
        <GenericList
          data={sortedData}
          renderItem={(plot) => <RegisteredPlotItem key={plot.parcel.id} data={plot} />}
        />
      ) : (
