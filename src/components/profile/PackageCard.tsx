...
const PackageCard: React.FC<PackageCardProps> = ({
    ...
    return (
        <div
            className='border-[1px] rounded-md p-6 shadow-md h-max w-[284px] border-gray-medium'
        >
            <p className="text-[14px] text-blue-muted">{title}</p>

            <div className='flex items-end my-5'>
                <p className={`text-[48px] font-semibold text-blue-muted leading-[44px] ${addClass}`}>{typeof price === 'number' ? `€${price}` : price}</p>
            ...
            <ul className="space-y-[0.9rem] text-gray-700">
                {features.map((feature, index) => (
                    <li key={index} className="flex items-center gap-2">
                        <span><img src={CheckCircleImg} alt="Check Circle" className='w-[16px] h-[16px]' /></span>
                        <span className='text-blue-muted text-[12px] flex items-center'>{feature.text} {feature.info && (
                            <span>
                                <img src={infoIcon} alt="Info Icon" className='w-[12px] h-[12px] ml-[0.4rem] cursor-pointer' />
                            </span>
                        )}</span>
                    </li>
                ))}
