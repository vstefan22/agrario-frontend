...
const OfferItem: FC<OfferItemProps> = ({ data }) => {
  ...
  const isAnalysePlus = data.parcels.length > 0 ? data.parcels[0].analyse_plus : false;
  ...
  return (
    ...
    >
      <div className="flex justify-between py-2 space-x-4">
        {data.parcels.length > 0 && <ItemImage id={data.parcels[0].id} image={imagePlaceholder} />}
        <div className="flex flex-col">
          ...
          <div className="flex justify-end items-center pt-5 gap-3">
            ...
            <Button variant="bluePrimary" type="button" onClick={handleViewDetails}>
              Angebots-Details
            </Button>
          </div>
        </div>
