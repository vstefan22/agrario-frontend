...
const RegisteredPlotItem: FC<RegisteredPlotItemProps> = ({ data }) => {
  ...
  const { registeredPlot, setRegisteredPlot, addPlotToMyList, updateRegisteredPlot } =
  ...
  const AddToWatchList = async () => {
    ...
    try {
      await addPlotToWatchlist(registeredPlot?.parcel.id.toString(), registeredPlot!);
      addPlotToMyList(registeredPlot!);
      navigate("/developer/my-watchlist");
    } catch (err) {
      toast.error("Das Flurstück wurde nicht erfolgreich zur Beobachtungsliste hinzugefügt.");
      console.error(err);
    }
  };
  ...
  const handleViewDetails = async () => {
    try {
      ...
      const plotRegistered = await getRegisteredPlotDetails(registeredPlot!.parcel.id.toString());
    ...
    >
      <div className="flex justify-between py-2 space-x-4">
        ...
        <div className="flex flex-col">
          <DynamicTable data={data.parcel} columns={PLOT_SEARCH_COLUMNS} customClassName="px-10" />
            ...
            <div>
              {(() => {
                ...
                if (restrictions.length > 0) {
                  return (
                    <>
                      <h1 className="text-black-muted text-[14px]">{restrictions.join(", ")}</h1>
